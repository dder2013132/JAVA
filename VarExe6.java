...
public class VarExe6 {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<4;i++) {
			if(members[i].name.equals(nm)) {
				System.out.println("조회 결과 : " + nm + ", " + members[i].score);
			}
		}
		int tmp = 0;
		String check[][] = new String[4][4] ;
		for(int i=0;i<4;i++) {
			if(members[i].score > tmp) {
				tmp = members[i].score;
			}
		}
		for(int i=0;i<4;i++) {
			if(members[i].score == tmp) {
				check[i][0] = members[i].name;
			}
		}
		for(int i=0;i<4;i++) {
			System.out.println("이름 : " + members[i].name + ", 점수 : " + members[i].score);
		}
		int i=0;
		while(i<4) {	
			if(check[i][0] != null) {
